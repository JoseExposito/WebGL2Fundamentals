...
// The example online transforms from angle (degrees) to X/Y in JavaScript
// We move it to the shader:
const vertexShaderSource = `#version 300 es
    ...
    uniform float u_angleInDegrees;

    void main() {
        float angleInRadians = u_angleInDegrees * M_PI / 180.0;
        float rotationX = sin(angleInRadians);
        float rotationY = cos(angleInRadians);
        vec2 rotatedPos = vec2(
            a_position.x * rotationY + a_position.y * rotationX,
            a_position.y * rotationY - a_position.x * rotationX
        );
        vec2 translatedPos = rotatedPos + u_translation;
...
const main = () => {
    ...
    const angleInDegreesUniformLocation = gl.getUniformLocation(program, 'u_angleInDegrees');
    ...
    // Set the rotation
    gl.uniform1f(angleInDegreesUniformLocation, 90);
